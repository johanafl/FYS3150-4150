...
double metropolis_flap(double** spin_mat, int row, int col, int n)
{   
    /*
    This is wrong!!!!!! But now it is also very different...
    */
    
    double spin_now   = spin[row][col];
    ...
    double r            = 0;    // NB!! THIS NEEDS TO BE A RANDOM NUMBER IN THE INTERVAL [0, 1]
    double energy       = spin_now*(spin_up + spin_down + spin_right + spin_left);
    double delta_energy = 2*(-energy)

    if r < cmath::exp(delta_energy)
    {
        spin_now *= -1
    }
